...
@Service
@ConditionalOnProperty(prefix = "sentinel.auth", name = "uniqueness-mode", havingValue = "username") 
public class UsernameAuthService extends BaseAuthService {
	
	public UsernameAuthService(SecurityProperties securityProperties,
	...
	protected BaseAccount findAccount(AccountRequest request) {
		return accountRepository.findByUsername(request.username()).orElseThrow(() -> {
